...
class dashboard_new extends MY_Controller {
	...
	public function index() {
		...
		// $sql = "SELECT count(*) as scmlts FROM user where usertype_id='7'";
		// $scmlts = $this->db->query($sql)->result_array();

		// $sql1 = "SELECT count(*) as clcs FROM user where usertype_id='13'";
		// $clcs = $this->db->query($sql1)->result_array();

		$data['title'] = "National Dashboard";
		$data['maps'] = json_encode($finalMap);
		$data['counties'] = $counties;
		// $data['scmlts'] = $scmlts[0]['scmlts'];
		// $data['clcs'] = $clcs[0]['clcs'];		
  ...
  //       $data['months_texts'] = $trend_data['months_texts'];         
		...
		$this -> load -> view("rtk/rtk/dashboard/national_v1.php", $data);

	}

    public function get_national_trend() {       
        ...
        $trend_data['first_month'] = date("M Y", strtotime( date( 'Y-m-01' )." -12 months")); 
        $trend_data['last_month'] = date("M Y", strtotime( date( 'Y-m-01' )." -1 months")); 
        $trend_data['percentages'] = $percentages;
        $trend_data['months_texts'] = $months_texts;
        $trend_data['trend_details'] = $trend_details;   
        return $trend_data;
    }
    public function get_national_stock_card() {  
    ...
    $table_head = '<table id="stock_card_table" class="data-table" width="25%"><thead>
    				<tr><th>Commodity Name</th><th>AMC</th><th>Stock on Hand at Facility</th><th>MOS Central</th></tr>
              		<tr><th colspan="4" id="th-banner">HIV Rapid Test Kit Stock Status as at end of '.$month_text.'</th></tr>
            		</thead>';
    ...
    $table_full = $table_head.$table_body.$table_foot;
    return $table_full;
    
}

public function get_national_expiries(){
